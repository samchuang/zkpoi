...
public class ArrayEval implements AreaEval {
	...
	private ArrayEval(Object[][] srcvalues, int firstRow, int firstColumn, int lastRow, int lastColumn) {
		_firstRow = firstRow;
	...
	public ArrayEval(ValueEval[][] srcvalues, int firstRow, int firstColumn, int lastRow, int lastColumn) {
		_firstRow = firstRow;
		_firstCol = firstColumn;
		_lastRow = lastRow;
		_lastCol = lastColumn;
	...
	public ArrayEval(ArrayPtg ptg) {
		this(ptg.getTokenArrayValues(), 0, 0, ptg.getRowCount() - 1, ptg.getColumnCount() - 1);
	}
	...
	public TwoDEval getColumn(int columnIndex) {
		...
		return new ArrayEval(tgtvalues, getFirstRow(), columnIndex, getLastRow(), columnIndex);
	}
	...
	public TwoDEval getRow(int rowIndex) {
		...
		return new ArrayEval(tgtvalues, rowIndex, getFirstColumn(), rowIndex, getLastColumn());
	}
